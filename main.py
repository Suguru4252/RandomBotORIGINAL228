...
def start_doctor_game(user_id, job_name):
    patients = [{'sym':'Боль в груди','act':['Нитроглицерин','Аспирин','Валидол'],'cor':0,'time':10},{'sym':'Температура, кашель','act':['Антибиотики','Парацетамол','Витамины'],'cor':1,'time':8}]
...
def start_cosmonaut_game(user_id, job_name):
    markup = types.InlineKeyboardMarkup(row_width=3)
    markup.row(types.InlineKeyboardButton("⬜", callback_data="cosmo_move_0_0"), types.InlineKeyboardButton("⬜", callback_data="cosmo_move_0_1"), types.InlineKeyboardButton("⬜", callback_data="cosmo_move_0_2"))
    markup.row(types.InlineKeyboardButton("⬜", callback_data="cosmo_move_1_0"), types.InlineKeyboardButton("🚀", callback_data="cosmo_pos_1_1"), types.InlineKeyboardButton("⬜", callback_data="cosmo_move_1_2"))
    markup.row(types.InlineKeyboardButton("⬜", callback_data="cosmo_move_2_0"), types.InlineKeyboardButton("⬜", callback_data="cosmo_move_2_1"), types.InlineKeyboardButton("🛸", callback_data="noop"))
    cosmonaut_games[user_id] = {'rocket':(1,1),'target':(2,2),'moves':0,'start':time.time()}
    return markup, f"👨‍🚀 **{job_name}**\n🛸 Проведи 🚀 к 🛸\n⏱️ Время пошло!"
...
def check_cosmonaut_move(user_id, x, y):
    ...
    if abs(rx-x)+abs(ry-y)==1:
        g['rocket']=(x,y); g['moves']+=1
        markup = types.InlineKeyboardMarkup(row_width=3)
        for i in range(3):
            row = []
            for j in range(3):
                if (i,j)==g['rocket']: row.append(types.InlineKeyboardButton("🚀", callback_data=f"cosmo_pos_{i}_{j}"))
                elif (i,j)==g['target']: row.append(types.InlineKeyboardButton("🛸", callback_data="noop"))
                else: row.append(types.InlineKeyboardButton("⬜", callback_data=f"cosmo_move_{i}_{j}"))
            markup.row(*row)
        if g['rocket']==g['target']:
            ts = time.time()-g['start']; del cosmonaut_games[user_id]
            score = max(100-g['moves']*5,70)
...
def handle(m):
    ...
    # ===== РАБОТЫ =====
        ...
        ok, rem = check_cooldown(uid, txt)
        if not ok: bot.send_message(uid, f"⏳ Подожди {rem} сек"); return
        
        if "Грузчик" in txt:
            mk, msg = start_loader_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Уборщик" in txt:
            mk, msg = start_cleaner_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Курьер" in txt:
            mk, msg = start_courier_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Механик" in txt:
            mk, msg = start_mechanic_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Программист" in txt:
            mk, msg = start_programmer_game(uid, txt)
            bot.send_message(uid, msg, parse_mode="Markdown", reply_markup=mk)
        elif "Детектив" in txt:
            mk, msg = start_detective_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Инженер" in txt:
            mk, msg = start_engineer_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Врач" in txt:
            mk, msg = start_doctor_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Артист" in txt:
            mk, msg = start_artist_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
        elif "Космонавт" in txt:
            mk, msg = start_cosmonaut_game(uid, txt)
            bot.send_message(uid, msg, reply_markup=mk)
...
def callback(call):
    ...
    if data.startswith("loader_"):
        ...
        if res['win']:
            job = "🚚 Грузчик"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Грузчик", res['score'], res['time'], reward)
            set_cooldown(uid, job)
            bot.edit_message_text(f"✅ **ПОБЕДА!**\n⏱️ {res['time']:.1f} сек\n💰 +{reward}\n⭐ +{exp}", uid, call.message.message_id)
        ...
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res['win']:
            job = "🧹 Уборщик"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Уборщик", res['score'], res['time'], reward)
            set_cooldown(uid, job)
            bot.edit_message_text(f"✅ **ПОБЕДА!**\n⏱️ {res['time']:.1f} сек\n💰 +{reward}\n⭐ +{exp}", uid, call.message.message_id)
        else: bot.answer_callback_query(call.id, f"✅ {res['collected']}/{res['total']}")
        ...
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res['win']:
            job = "📦 Курьер"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Курьер", res['score'], res['time'], reward)
            set_cooldown(uid, job)
        ...
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res.get('win'):
            job = "🔧 Механик"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Механик", res['score'], res['time'], reward)
            set_cooldown(uid, job)
        ...
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res['win']:
            job = "💻 Программист"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Программист", res['score'], res['time'], reward)
            set_cooldown(uid, job)
        ...
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res['win']:
            job = "🕵️ Детектив"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Детектив", res['score'], res['time'], reward)
            set_cooldown(uid, job)
        ...
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res.get('win'):
            job = "👨‍🔧 Инженер"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Инженер", res['score'], res['time'], reward)
            set_cooldown(uid, job)
        ...
        p = data.split("_"); cor, lim = p[1], int(p[2]); res = check_doctor_choice(uid, cor, lim)
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res['win']:
            job = "👨‍⚕️ Врач"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Врач", res['score'], res['time'], reward)
            set_cooldown(uid, job)
            bot.edit_message_text(f"✅ **ПАЦИЕНТ СПАСЕН!**\n⏱️ {res['time']:.1f} сек\n💰 +{reward}\n⭐ +{exp}", uid, call.message.message_id)
        else: bot.edit_message_text("❌ **ПАЦИЕНТ УМЕР**\nПопробуй еще!", uid, call.message.message_id)
    
    elif data.startswith("artist_"):
        cor = data.split("_")[1]; res = check_artist_choice(uid, cor)
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res['win']:
            job = "👨‍🎤 Артист"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Артист", res['score'], res['time'], reward)
            set_cooldown(uid, job)
            bot.edit_message_text(f"✅ **ПРАВИЛЬНО!**\n⏱️ {res['time']:.1f} сек\n💰 +{reward}\n⭐ +{exp}", uid, call.message.message_id)
        else: bot.edit_message_text("❌ **НЕ УГАДАЛ**\nПопробуй еще!", uid, call.message.message_id)
    
    elif data.startswith("cosmo_move_"):
        p = data.split("_"); x, y = int(p[2]), int(p[3]); res = check_cosmonaut_move(uid, x, y)
        if not res: bot.answer_callback_query(call.id, "❌ Игра не найдена"); return
        if res.get('win'):
            job = "👨‍🚀 Космонавт"; job_data = conn.execute('SELECT min_reward, max_reward, exp_reward FROM jobs WHERE job_name = ?', (job,)).fetchone()
            reward = random.randint(job_data[0], job_data[1]); exp = job_data[2]
            add_balance(uid, reward); add_exp(uid, exp)
            update_work_stats(uid, "Космонавт", res['score'], res['time'], reward)
            set_cooldown(uid, job)
            bot.edit_message_text(f"✅ **СТЫКОВКА УСПЕШНА!**\n⏱️ {res['time']:.1f} сек\n💰 +{reward}\n⭐ +{exp}", uid, call.message.message_id)
        elif res.get('moved'): bot.edit_message_reply_markup(uid, call.message.message_id, reply_markup=res['markup'])
        else: bot.answer_callback_query(call.id, "❌ Нельзя")
